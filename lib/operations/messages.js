...
class Messages {
   ...
   *
   * @param {number} [options.pollFor] Number of seconds to poll for a result.
   * 
   * @param {date} [options.receivedAfter] Limits results to only messages 
   * receieved after this date/time.
   *
  ...
  get(server, criteria, options, optionalCallback) {
    ...
    options.pollFor = options.pollFor || 5;
  ...
  search(server, criteria, options, optionalCallback) {
    ...
    if (!Number.isInteger(options.pollFor)) {
      options.pollFor = 0;
    }

    if (options.pollFor > 30) {
      options.pollFor = 30;
    }
    ...
    if (!optionalCallback) {
      let fn = (resolve, reject) => function() {
        self.client.request.post(url, { qs: qs, body: criteria }, (err, response, body) => {
          if (err || response.statusCode !== 200 || pollCount > options.pollFor) {
              return reject(new MailosaurError(response));
          }

          if (options.pollFor && !body.items.length) {
            pollCount++;
            return setTimeout(fn(resolve, reject), 1000);
          }
