...
class Messages {
   ...
   *                      See {@link Message} for more information.
   */
waitFor(server, criteria, optionalCallback, timeout=15) {
  const waitForPromise = new Promise(async (resolve, reject) => {    
    let timeoutDatetime = new Date();
    timeoutDatetime.setSeconds(timeoutDatetime.getSeconds() + timeout);

    while(Date.now() < timeoutDatetime.getTime()) {
      const messages = await this.search(server, criteria);
      if(messages.items.length > 0) {
        const message = await this.get(messages.items[0].id)
    ...
    return reject(new MailosaurError({
      statusCode: 400,
      body: {
  ...
  if (!optionalCallback) {
    ...
    waitForPromise
    .then(result => optionalCallback(result))
    .catch(error => optionalCallback(null, error));
  }
}
