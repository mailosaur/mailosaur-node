...
class Messages {
   ...
   *                      {null} [result]   - The deserialized result object if an error did not occur.
   */
  create(server, options, optionalCallback) {
    const self = this;
    const url = `api/messages`;
    ...
    if (!optionalCallback) {
      return new Promise((resolve, reject) => {
        self.client.request.post(url, { qs, body: options }, (err, response) => {
          if (err || response.statusCode !== 204) {
            return reject(err || self.client.httpError(response));
          }
          resolve();
   ...
   *                      {null} [result]   - The deserialized result object if an error did not occur.
   */
  forward(id, options, optionalCallback) {
    const self = this;
    const url = `api/messages/${id}/forward`;

    if (!optionalCallback) {
      return new Promise((resolve, reject) => {
        self.client.request.post(url, { body: options }, (err, response) => {
          if (err || response.statusCode !== 204) {
            return reject(err || self.client.httpError(response));
          }
          resolve();
   ...
   *                      {null} [result]   - The deserialized result object if an error did not occur.
   */
  reply(id, options, optionalCallback) {
    const self = this;
