...
class Messages {
   ...
   * result (in milliseconds).
   *
   * @param {number} [options.errorOnTimeout] When set to false, an error will
          ...
          if (options.timeout && !body.items.length) {
            ...
            if (((Date.now() - startTime) + delay) > options.timeout) {
              return (options.errorOnTimeout === false) ?
                ...
                optionalCallback(new MailosaurError('No matching messages found in time. By default, only messages received in the last hour are checked (use receivedAfter to override this).', 'search_timeout'));
            }
   ...
   *                      {null} [result]   - The deserialized result object if an error did not occur.
   */
  create(server, messageCreateOptions, optionalCallback) {
    const self = this;
    const url = `api/messages`;
    ...
    if (!optionalCallback) {
      return new Promise((resolve, reject) => {
        self.client.request.post(url, { qs, body: messageCreateOptions }, (err, response, body) => {
          if (err || response.statusCode !== 200) {
            return reject(err || self.client.httpError(response));
          }
          resolve(new Message(body));
   ...
   *                      {null} [result]   - The deserialized result object if an error did not occur.
   */
  forward(id, messageForwardOptions, optionalCallback) {
    const self = this;
    const url = `api/messages/${id}/forward`;

    if (!optionalCallback) {
      return new Promise((resolve, reject) => {
        self.client.request.post(url, { body: messageForwardOptions }, (err, response, body) => {
          if (err || response.statusCode !== 200) {
            return reject(err || self.client.httpError(response));
          }
          resolve(new Message(body));
   ...
   *                      {null} [result]   - The deserialized result object if an error did not occur.
   */
  reply(id, messageReplyOptions, optionalCallback) {
    const self = this;
    const url = `api/messages/${id}/reply`;

    if (!optionalCallback) {
      return new Promise((resolve, reject) => {
        self.client.request.post(url, { body: messageReplyOptions }, (err, response, body) => {
          if (err || response.statusCode !== 200) {
            return reject(err || self.client.httpError(response));
          }
          resolve(new Message(body));
